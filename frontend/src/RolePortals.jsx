...
function ProfileScreen() {
  ...
  const uploadDocument = async ({ documentType, file }) => {
    ...
    try {
      ...
      if (!uploadResponse.ok) {
        throw new Error(`Kho lưu trữ từ chối tệp tải lên (${uploadResponse.status}).`);
      }
  ...
  const deleteDocument = async () => {
    ...
    try {
      ...
      setMessage(`Đã xóa “${documentToDelete.fileName}” khỏi hồ sơ và kho lưu trữ.`);
    } catch (requestError) {
      ...
      <div className="modal portal-modal candidate-decision-modal" role="dialog" aria-modal="true" onMouseDown={event => event.stopPropagation()}>
        ...
        {isInterview ? (
          ...
          <div className="modal-form two-cols interview-form">
            ...
            <label className="offer-file-picker"><span>PDF offer (không bắt buộc)</span><input type="file" accept="application/pdf,.pdf" onChange={event => setOfferFile(event.target.files?.[0] || null)} /><small>{offerFile ? `${offerFile.name} · ${formatBytes(offerFile.size)}` : "Tối đa 10 MB. Tệp được lưu trữ riêng tư."}</small></label>
