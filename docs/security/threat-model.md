...
| LOG-01 | Secret/PII/presigned URL xuất hiện trong log (I/R) | App không log body hay header theo mặc định; lỗi chuẩn chỉ trả `traceId`; email template escape HTML. Xem [`error-handler.ts`](../../backend/src/middleware/error-handler.ts), [`request-context.ts`](../../backend/src/middleware/request-context.ts), [`email-template.ts`](../../backend/src/modules/email/email-template.ts). | **Cao**: unexpected error hiện dùng `console.error(error)` chưa redaction/structured sink; cần logger có allow-list, retention và alert. |
...
| CONFIG-01 | Demo account/secret còn tồn tại khi mở cho user thật (S/E) | Production bắt buộc JWT secret >= 32 ký tự; demo reset bị chặn theo environment/flag; secret không đưa sang frontend. Xem [`env.ts`](../../backend/src/config/env.ts), [`vercel-neon.md`](../deployment/vercel-neon.md). | **Cao** cho đến khi có bằng chứng `VITE_SHOW_DEMO_ACCOUNTS=false`, xóa/khóa demo users và rotate JWT/Cron/R2/Resend/DB credentials trước go-live. |
| DB-01 | Mất dữ liệu hoặc restore không dùng được (DoS/T) | Migration có version; production cấm demo reset; workflow yêu cầu database test/E2E riêng. | **Cao**: chưa có bằng chứng restore. Phải diễn tập backup/restore trên Neon branch test, ghi RPO/RTO/checksum/count; tuyệt đối không restore/reset production. |
| OBS-01 | Sự cố/attack không được phát hiện (R) | Mỗi response có `x-request-id` và error envelope có `traceId`; Vercel có runtime logs. | **Cao**: chưa có error monitor, metric, alert và correlation end-to-end. Cần Sentry/OTel hoặc tương đương cùng quy tắc redaction. |
